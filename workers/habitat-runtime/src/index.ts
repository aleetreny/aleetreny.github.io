...
import { adminCommandSchema, cognitionJobSchema } from './contracts';
...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    if (request.method === 'OPTIONS' && (url.pathname === '/health' || url.pathname === '/v1/status')) {
    ...
    if (request.method === 'GET' && url.pathname === '/health') {
      return publicJson({ ok: true, service: 'habitat-runtime' }, env);
    }
