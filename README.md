...
Portfolio personal presentado como un **working board**: una pizarra finita colgada de una pared, arrastrable y ampliable (pan, zoom, teclado, táctil), donde el trabajo, los estudios, el laboratorio, el voluntariado, los hackathones, el código, los viajes y las obsesiones aparecen como tarjetas de papel giradas que se abren en dossiers a página completa. Cajones, polaroids con foto y notas adhesivas conviven en el mismo tablero, con siete texturas de pizarra y siete paredes de fondo. El repositorio es la fuente principal: una máquina nueva debe poder clonar, instalar, configurar servicios, migrar y desplegar sin recuperar archivos locales anteriores.

La primera visita no ve el tablero de golpe: la pizarra **se estampa contra la pared** y el visitante lo recorre parada a parada, a su ritmo, mientras cada cajón, polaroid y nota se va clavando encima. Al terminar, el tablero es exactamente el que se publica —pan, zoom, arrastre, dossiers, barra de herramientas—. El recorrido se salta con `Escape` o `skip`, se repite con `↻ tour`, y no se ejecuta nunca con `prefers-reduced-motion: reduce` ni en modo propietario.

En **móvil** el tablero es táctil de verdad: pellizco para el zoom, dos dedos para desplazar, doble toque para encuadrar una tarjeta. Como el lienzo mide 2540px, por debajo de 720px de ancho (o 460px de alto, con el teléfono tumbado) el recorrido se camina de pieza en pieza con márgenes de teléfono, la vista de reposo encuadra la primera tarjeta en lugar de un tablero ilegible, y la barra de herramientas se reduce a una fila deslizable. Todos esos umbrales son editables.

La web pública es una SPA estática. El catálogo versionado (`fixtures/demo-content.json`, generado con `pnpm content:build`) contiene los dossiers reales agrupados en nueve cajones y la configuración del tablero (`fixtures/site-settings.json`: `theme`, `board`, `board.layout` y `board.tour`). Con `VITE_ENABLE_REMOTE_DATA=false` usa esos fixtures; con Neon configurado lee contenido y ajustes publicados y conserva la copia segura si el servicio no responde. El **modo propietario** vive en `?owner=1` (o con el atajo `Shift+E`): autentica con Managed Better Auth y obtiene permisos editoriales solo si el UUID está en `app_private.owner_accounts`. Una vez dentro se edita **todo desde la propia web, sin tocar código**: texto en línea de cada tarjeta y dossier, posición de las tarjetas (drag, tidy, scatter, reset), colores, tipografías, textura y fondo desde el panel *theme*, el recorrido completo desde el panel *tour*, fotos (subida a Neon Object Storage) y alta/baja de dossiers con papelera recuperable. Los cambios persisten en Neon (`content_entries`/`content_blocks` vía RPC transaccional con bloqueo optimista, y `site_settings` para tema, layout y recorrido).
...
### Sembrar el contenido en Neon

El workflow manual `.github/workflows/seed-content.yml` carga el catálogo versionado y los ajustes del tablero en el `DATABASE_URL` del Environment elegido (aplica migraciones, siembra y verifica). Con `replace_catalogue=true` mueve a la papelera recuperable cualquier entrada que no forme parte del catálogo, para reemplazar un catálogo anterior en la primera siembra. `production` exige `production_confirmation=APPLY_PRODUCTION`. Regenera los fixtures con `pnpm content:build` antes de sembrar si has cambiado las fuentes en `content/source/`.
...
La fase actual deja terminados el frontend público, el editor visual, la persistencia versionada, el broker de imágenes y la automatización. En el proyecto Neon definitivo de `aws-us-east-2`, Auth, Data API, migraciones, RLS, Storage, Function, flujo editorial y ciclo completo de archivos han pasado en integración y producción. La cuenta propietaria real está allowlisted y los environments `development` y `production` tienen sus Secrets; ambos planes de infraestructura pasaron en GitHub Actions el 2026-08-05. El catálogo ampliado está sembrado en las dos ramas Neon. Consulta el estado de la revisión online y de la release en [PROJECT_STATUS.md](PROJECT_STATUS.md).
