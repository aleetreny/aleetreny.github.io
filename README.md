...
Portfolio personal presentado como un archivo de investigación sobre corcho: una superficie arrastrable y ampliable donde proyectos, experimentos, experiencia, formación y trabajo cultural aparecen como dossiers técnicos que se pueden abrir. El repositorio es la fuente principal: una máquina nueva debe poder clonar, instalar, configurar servicios, migrar y desplegar sin recuperar archivos locales anteriores.

La web pública es una SPA estática con pan, zoom, controles de teclado/táctiles, índice cartográfico y diez secciones expandibles dentro de la misma estética de archivo. El catálogo versionado contiene 24 entradas públicas verificables agrupadas en investigación, IA, inteligencia cívica, productos, experimentos, trayectoria, formación, comunidad, perfil y contacto. Con `VITE_ENABLE_REMOTE_DATA=false` usa esos fixtures; con Neon configurado lee únicamente contenido publicado y conserva la copia segura si el servicio no responde. El modo propietario vive en `?owner=1`: autentica con Managed Better Auth y obtiene permisos editoriales solo si el UUID está en `app_private.owner_accounts`. Incluye CRUD de entradas, papelera recuperable, editor de bloques reordenable, subida de imágenes, bloqueo optimista e historial restaurable, usando el mismo sistema visual que el frontend.
