...
Portfolio personal presentado como un **working board** sobre una pizarra oscura: una superficie arrastrable y ampliable (pan, zoom, teclado, táctil) donde el trabajo, los estudios, el laboratorio, el voluntariado, los hackathones, el código, los viajes y las obsesiones aparecen como tarjetas de papel giradas que se abren en dossiers a página completa. Cajones, polaroids con foto y notas adhesivas conviven en el mismo tablero, con cuatro texturas de fondo (blueprint, cork, graphite, slate). El repositorio es la fuente principal: una máquina nueva debe poder clonar, instalar, configurar servicios, migrar y desplegar sin recuperar archivos locales anteriores.

La web pública es una SPA estática. El catálogo versionado (`fixtures/demo-content.json`, generado con `pnpm content:build`) contiene los dossiers reales agrupados en nueve cajones y la configuración del tablero (`fixtures/site-settings.json`: tema, tarjetas, layout). Con `VITE_ENABLE_REMOTE_DATA=false` usa esos fixtures; con Neon configurado lee contenido y ajustes publicados y conserva la copia segura si el servicio no responde. El **modo propietario** vive en `?owner=1` (o con el atajo `Shift+E`): autentica con Managed Better Auth y obtiene permisos editoriales solo si el UUID está en `app_private.owner_accounts`. Una vez dentro se edita **todo desde la propia web, sin tocar código**: texto en línea de cada tarjeta y dossier, posición de las tarjetas (drag, tidy, scatter, reset), colores, tipografías y textura desde el panel *theme*, fotos (subida a Neon Object Storage) y alta/baja de dossiers con papelera recuperable. Los cambios persisten en Neon (`content_entries`/`content_blocks` vía RPC transaccional con bloqueo optimista, y `site_settings` para tema y layout).
