...
export function InventoryPanel({
  ...
  async function create() {
    ...
    if (!clean) { notify('Write a title first.', true); return; }
    setBusy(true);
    try {
      ...
      onCreated(saved);
      setTitle('');
      notify('Dossier created.');
    } catch (error) {
      notify(error instanceof Error ? error.message : 'Could not create it.', true);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function remove(entry: PortfolioEntry) {
    if (!window.confirm(`Send "${entry.title}" to the trash?`)) return;
    try {
      ...
      onDeleted(entry.id);
      notify('Moved to the trash.');
    } catch (error) {
      notify(error instanceof Error ? error.message : 'Could not delete it.', true);
    }
  }
  ...
  async function restore(item: DeletedEntrySummary) {
    try {
      ...
      notify(error instanceof Error ? error.message : 'Could not restore it.', true);
    }
  }
  ...
  function deleteList(id: string) {
    ...
    if (remaining.length === 0) { notify('At least one list has to remain.', true); return; }
    const label = labelFor(id);
    const affected = entriesForGroup(entries, id);
    const fallback = remaining.find((g) => g.id === 'random')?.id ?? remaining[0].id;
    if (affected.length > 0) {
      const ok = window.confirm(`"${label}" holds ${affected.length} dossier(s); they will move to "${labelFor(fallback)}". Delete the list?`);
      if (!ok) return;
      for (const entry of affected) onMoveEntry(entry, fallback);
    } else if (!window.confirm(`Delete the list "${label}"?`)) {
      return;
    }
  ...
  return (
    <div className="overlay" role="presentation">
      ...
      <div className="panel panel--theme" role="dialog" aria-modal="true" aria-label="Manage dossiers">
        ...
        <div style={{ maxHeight: 200, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
          {board.groups.map((g) => (
            <div key={g.id} className="field-row" style={{ margin: 0 }}>
              ...
              <button className="editdel" type="button" onClick={() => deleteList(g.id)} aria-label={`Delete the list ${g.label}`}>×</button>
            </div>
          ))}
        </div>
        <div className="field-row">
        ...
        <div style={{ maxHeight: 180, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
          {board.groups.map((g) => entriesForGroup(entries, g.id).map((entry) => (
            <div key={entry.id} className="field-row" style={{ margin: 0 }}>
              ...
              <button className="editdel" type="button" onClick={() => remove(entry)} aria-label={`Delete ${entry.title}`}>×</button>
            </div>
          )))}
        </div>
