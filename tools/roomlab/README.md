...
| `workshops.html` | **The Workshops** — the first rock-side room. Traced off `workshop-two-bay.jpg` for its composition and every prop, and off `workshop-plate-walls-on-dirt.png` for the wall and floor of a room that is not a built room. Three bays divided by bolted plate, the fabricator, and the queue. |
